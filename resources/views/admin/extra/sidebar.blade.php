    ...
    <nav class="flex-1 px-3 space-y-1 mt-6 overflow-x-hidden custom-scrollbar">
        ...
        <a href="{{ route('admin.index') }}" class="nav-item flex items-center gap-4 p-3 rounded-xl transition-all duration-300 group relative 
           {{ Route::is('admin.index') ? 'active text-white' : 'text-gray-400 hover:bg-white/5 hover:text-white' }}">
            ...
            <span
                class="sidebar-text font-medium whitespace-nowrap opacity-100 transition-all duration-300">Dashboard</span>
            <div
                class="active-indicator absolute left-0 w-1 h-8 bg-white rounded-r-full {{ Route::is('admin.index') ? '' : 'hidden' }}">
            </div>
        </a>
        ...
        <a href="{{ route('admin.credit') }}" class="nav-item flex items-center gap-4 p-3 rounded-xl transition-all duration-300 group relative 
           {{ Route::is('admin.credit') ? 'active text-white' : 'text-gray-400 hover:bg-white/5 hover:text-white' }}">
            <i
                class="fa-solid fa-credit-card text-xl min-w-[40px] text-center group-hover:scale-110 transition-transform"></i>
            <span class="sidebar-text font-medium whitespace-nowrap opacity-100 transition-all duration-300">Credit
                Details</span>
            <div
                class="active-indicator absolute left-0 w-1 h-8 bg-white rounded-r-full {{ Route::is('admin.credit') ? '' : 'hidden' }}">
            </div>
        </a>
        ...
        <a href="{{ route('admin.profile') }}" class="nav-item flex items-center gap-4 p-3 rounded-xl transition-all duration-300 group relative 
           {{ Route::is('admin.profile') ? 'active text-white' : 'text-gray-400 hover:bg-white/5 hover:text-white' }}">
            ...
            <span
                class="sidebar-text font-medium whitespace-nowrap opacity-100 transition-all duration-300">{{ Auth::user()->name }}'s
                Profile</span>
            <div
                class="active-indicator absolute left-0 w-1 h-8 bg-white rounded-r-full {{ Route::is('admin.profile') ? '' : 'hidden' }}">
            </div>
        </a>

        <a href="{{ route('gallery.page') }}" class="nav-item flex items-center gap-4 p-3 rounded-xl transition-all duration-300 group relative 
   {{ Route::is('gallery.page') ? 'bg-blue-600 text-white' : 'text-gray-400 hover:bg-white/5 hover:text-white' }}">

            <i class="fa-solid fa-image text-xl min-w-[40px] text-center group-hover:scale-110 transition-transform"></i>

            <span class="sidebar-text font-medium whitespace-nowrap opacity-100 transition-all duration-300">
                {{ Auth::user()->name }}'s Gallery
            </span>
            <div class="active-indicator absolute left-0 w-1 h-8 bg-blue-600 rounded-r-full 
        {{ Route::is('gallery.page') ? '' : 'hidden' }}">
            </div>
        </a>
        ...
        <a href="{{ route('admin.smartselection') }}"
           ...
           {{ Route::is('admin.smartselection') ? 'active text-white' : 'text-gray-400 hover:bg-white/5 hover:text-white' }}">
            ...
            <span
                class="sidebar-text absolute right-2 bg-blue-500 text-[9px] px-1.5 py-0.5 rounded font-bold uppercase shadow-sm">New</span>
            <div
                class="active-indicator absolute left-0 w-1 h-8 bg-white rounded-r-full {{ Route::is('admin.smartselection') ? '' : 'hidden' }}">
            </div>
        </a>
    </nav>
    ...
    <div class="p-4 border-t border-white/5 mt-auto bg-[#151725]">
        ...
        <form method="POST" action="{{ route('logout') }}">
            ...
            <button type="submit"
                ...
                <i
                    class="fa-solid fa-right-from-bracket text-xl min-w-[40px] text-center group-hover:-translate-x-1 transition-transform"></i>

                <span class="sidebar-text whitespace-nowrap opacity-100 transition-all duration-300">
                    Sign Out
                </span>
