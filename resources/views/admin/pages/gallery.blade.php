...
@section('content')
    <div id="uploadLoader"
        class="fixed inset-0 z-[200] hidden flex-col items-center justify-center bg-white/80 backdrop-blur-md">
        <div class="relative">
        ...
        <p class="text-sm text-gray-500 font-medium text-center px-6">Please wait, we are optimizing and uploading your
            high-quality photos (Up to 10MB).</p>
    ...
    <form action="{{ route('admin.gallery.update', $gallery->studio->id) }}" method="POST" enctype="multipart/form-data"
        id="galleryUpdateForm">
        @method('PUT')
        @csrf
        ...
        <div class="flex h-screen overflow-hidden bg-[#f4f7fe]">
            ...
            <div id="main-panel"
                class="flex-1 flex flex-col min-w-0 overflow-hidden transition-all duration-500 ease-in-out">
                @include('admin.extra.header')

                <div class="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">

                    <div
                        class="mb-8 flex justify-between items-center bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
                    ...
                    <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-8 space-y-8">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            <div class="space-y-2">
                                ...
                                <input type="text" name="studio_name"
                                    value="{{ old('studio_name', $gallery->studio->studio_name ?? '') }}"
                            ...
                            <div class="space-y-2">
                                ...
                                <input type="text" name="contact_person"
                                    value="{{ old('contact_person', $gallery->studio->contact_person ?? '') }}"
                                    class="w-full border-gray-200 rounded-xl p-3 text-sm focus:ring-2 focus:ring-indigo-400 outline-none border transition-all">
                            </div>
                            <div class="space-y-2">
                                <label class="text-xs font-bold text-gray-500 uppercase">Email</label>
                                <input type="email" name="studio_email"
                                    value="{{ old('studio_email', $gallery->studio->studio_email ?? '') }}"
                                    class="w-full border-gray-200 rounded-xl p-3 text-sm focus:ring-2 focus:ring-indigo-400 outline-none border transition-all">
                            </div>
                            <div class="space-y-2">
                            ...
                            <div class="space-y-2">
                                ...
                                <input type="text" name="experience"
                                    value="{{ old('experience', $gallery->studio->experience ?? '') }}"
                                    class="w-full border-gray-200 rounded-xl p-3 text-sm focus:ring-2 focus:ring-indigo-400 outline-none border transition-all">
                            </div>
                        </div>
                        ...
                        <div class="flex items-start gap-8 bg-gray-50/50 p-6 rounded-2xl border border-gray-100">
                            <div
                                class="relative group h-32 w-48 bg-gray-100 rounded-2xl overflow-hidden border border-gray-200 flex items-center justify-center shrink-0 shadow-inner">
                                <img id="coverPreview"
                                    src="{{ !empty($gallery->studio->album->cover_photo) ? asset('web/media/lg/' . $gallery->studio->album->cover_photo) : '' }}"
                                    class="w-full h-full object-cover {{ !empty($gallery->studio->album->cover_photo) ? '' : 'hidden' }}">
                                <div id="coverPlaceholder"
                                    class="{{ $gallery->studio->album->cover_photo ? 'hidden' : '' }} text-center p-3">
                                ...
                                <button type="button" id="btnRemoveCover"
                                    class="absolute top-2 right-2 bg-red-500 text-white w-7 h-7 rounded-full flex items-center justify-center shadow-lg opacity-0 group-hover:opacity-100 transition-all text-xs">
                            ...
                            <div class="space-y-3 flex-1">
                                ...
                                <p class="text-[10px] text-gray-400 font-medium italic mt-2">Recommended: 1920x1080px
                                    (16:9). Images up to 10MB are optimized.</p>
                            </div>
                        </div>
                            ...
                            <div class="space-y-4">
                                <div>
                                    ...
                                    <input type="text" name="album_name"
                                        value="{{ old('album_name', $gallery->studio->album->album_name ?? '') }}"
                                        class="w-full border-gray-200 rounded-xl p-3 text-sm focus:ring-2 focus:ring-indigo-400 outline-none border transition-all">
                                </div>
                                <div class="space-y-2">
                                    <label class="text-xs font-bold text-gray-500 uppercase tracking-wider">Album Unique
                                        Code</label>
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 pt-4 border-t border-gray-50">
                            <div class="space-y-4">
                                <label class="text-xs font-bold text-gray-500 uppercase">Background Music (MP3 - Max
                                    10MB)</label>
                                <div class="flex items-center gap-3">
                                    ...
                                    <button type="button" id="btnRemoveSong"
                                        class="{{ isset($gallery->studio->album->album_song) ? '' : 'hidden' }} w-12 h-12 bg-red-100 text-red-600 rounded-2xl flex items-center justify-center hover:bg-red-500 hover:text-white transition-all shadow-sm">
                                        <i class="fa-solid fa-trash-can"></i>
                                    </button>
                                    <input type="hidden" name="remove_song" id="removeSongInput" value="0">
                                </div>
                                <audio id="songPreview" controls
                                    class="{{ isset($gallery->studio->album->album_song) ? '' : 'hidden' }} w-full h-10 mt-2 rounded-lg bg-white">
                                    @if(isset($gallery->studio->album->album_song))
                                        <source src="{{ asset('storage/songs/' . $gallery->studio->album->album_song) }}"
                                            type="audio/mpeg">
                                    @endif
                                </audio>
                            </div>
                            ...
                            <div class="space-y-4">
                                <div class="flex items-center justify-between">
                                    <label class="text-xs font-bold text-gray-500 uppercase">Add Gallery Photos (Max 10MB
                                        Each)</label>
                                    <span id="imageCountLabel"
                                        class="text-[10px] font-bold text-indigo-500 bg-indigo-50 px-3 py-1 rounded-full uppercase">0
                                        Selected</span>
                                </div>
                                <div
                                    ...
                                    <input type="file" id="albumImages" name="album_photos[]" multiple="multiple"
                                        accept="image/*" class="hidden">
                                    <label for="albumImages" class="cursor-pointer flex flex-col items-center">
                                        <i
                                            class="fa-solid fa-images text-3xl text-indigo-300 group-hover:text-indigo-600 mb-2"></i>
                                        <span
                                            class="bg-indigo-600 text-white px-6 py-2 rounded-xl text-xs font-bold shadow-xl shadow-indigo-100">Upload
                                            Photos</span>
                                    </label>
                                </div>
                            </div>
                    ...
                    <h4 class="font-bold text-gray-700 flex items-center gap-2 pt-6"><i
                            class="fa-solid fa-box-archive text-indigo-500"></i> Active Gallery</h4>
                    <div id="removedImagesContainer"></div>

                    @if($gallery && is_array($gallery->images))
                        <div id="currentGalleryGrid"
                            class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6 pb-20">
                            @foreach($gallery->images as $image)
                                <div class="gallery-item group relative aspect-square bg-white rounded-2xl overflow-hidden border border-gray-100 shadow-sm hover:shadow-xl transition-all"
                                    data-image-path="{{ $image }}">
                                    <img src="{{ asset('web/media/sm/' . $image) }}" class="w-full h-full object-cover">
                                    <button type="button"
                                        class="btn-remove-image absolute top-2 right-2 bg-red-500 text-white w-7 h-7 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity shadow-lg">
